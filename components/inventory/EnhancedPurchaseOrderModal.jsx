  ...
  const addToCart = (item, unitCost = null) => {
    ...
    if (existingItemIndex >= 0) {
      ...
      setCart([...cart, {
        id: item.id,
        name: item.name,
        unit: item.unit || 'pcs', // Default to 'pcs' if unit is undefined
    ...
    try {
      ...
      const formattedData = {
        ...
        items: cart.map(item => ({
          ...
          itemName: item.name,
          itemUnit: item.unit || 'pcs', // Default to 'pcs' if unit is undefined
      ...
      >
        <div className="h-full flex flex-col bg-gradient-to-br from-white/95 to-gray-50/95 backdrop-blur-xl">
            ...
            <div className="hidden lg:flex items-center space-x-6">
              {[1, 2, 3].map((step) => (
                <div key={step} className="flex items-center">
                  <div
                    className={`flex items-center space-x-3 cursor-pointer transition-all duration-200 ${currentStep >= step ? 'text-black' : 'text-gray-400'
                      }`}
                    onClick={() => goToStep(step)}
                  >
                    <div className={`w-10 h-10 rounded-full flex items-center justify-center text-sm font-bold transition-all duration-200 ${currentStep >= step
          ...
          <div className="flex-1 overflow-hidden">
            ...
            {currentStep === 2 && (
              <div className="h-full flex flex-col xl:flex-row overflow-y-auto">
                {/* Product Catalog - Responsive width */}
                <div className="flex-1 flex flex-col w-full xl:w-1/2 min-h-[50vh] xl:min-h-0 border-b xl:border-b-0 xl:border-r border-white/20">
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                        {filteredItems.map(item => {
                          ...
                          return (
                            <div
                              key={item.id}
                              className={`group relative bg-white/90 backdrop-blur-sm border rounded-xl p-4 cursor-pointer hover:shadow-2xl transition-all duration-300 ${isInCart
                            ...
                            >
                              ...
                              <div className="absolute top-2 right-2 flex flex-col space-y-1">
                                ...
                                {isInCart && (
                                  <div className="w-2 h-2 bg-green-500 rounded-full" title="In Order"></div>
                                )}
                              </div>
                ...
                {/* Enhanced Order Summary - Adjusted for tablet responsiveness */}
                <div className="w-full xl:w-1/2 bg-white/70 backdrop-blur-sm flex flex-col border-t xl:border-t-0">
                  ...
                  {cart.length > 0 && (
                    <div className="p-4 border-t border-white/20 bg-white/90 space-y-4">
                      ...
                      {/* Action Buttons - Removed Redundant Continue Button */}
                      <div className="space-y-2">
                        {/* 
                          We only show the "Continue" here if it's not mobile/tablet 
                          where the footer button would be more accessible, 
                          but since the user only wanted 1 button, 
                          we keep this one as the primary one.
                        */}
                        <Button
                          onClick={nextStep}
