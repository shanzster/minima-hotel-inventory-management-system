...
export default function NotificationDropdown() {
  ...
  // Fetch real-time notifications based on inventory conditions
  useEffect(() => {
    const fetchNotifications = async () => {
      try {
        const [inventoryItems, pendingOrders] = await Promise.all([
          inventoryApi.getAll(),
        ...
        if (pendingOrders.length > 0 && ['inventory-controller', 'purchasing-officer'].includes(user?.role)) {
          newNotifications.push({
            ...
            details: pendingOrders.slice(0, 2).map(po => `${po.orderNumber} - ${formatCurrency(po.totalAmount)}`).join(', '),
            timestamp: now,
            priority: 'medium',
            icon: '📋',
