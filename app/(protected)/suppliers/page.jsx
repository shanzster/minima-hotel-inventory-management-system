...
export default function SuppliersPage() {
  ...
  const exportToExcel = async () => {
    try {
      setIsExporting(true)
      const { exportSuppliersToExcel } = await import('../../../lib/excelExportUtils')
      const suppliersData = filteredSuppliers.map(s => ({
        ...s,
        status: !s.isApproved ? 'Pending Approval' : s.isActive ? 'Active' : 'Inactive',
        performanceRating: s.performanceMetrics?.overallRating || 'N/A'
      }))
      await exportSuppliersToExcel(suppliersData)
    } catch (error) {
          ...
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center space-x-3 ml-4">
              ...
              <button
                ...
                className="inline-flex items-center px-4 py-2 bg-green-600 text-white rounded-lg text-sm hover:bg-green-700 hover:shadow-lg hover:scale-105 transition-all duration-200 ease-out disabled:opacity-50"
              >
                {isExporting ? (
                  <>
