...
export const inventoryApi = {
  ...
  // Get transactions for a specific item (including bundle consumption)
  async getTransactions(itemId) {
    ...
    try {
      // Import bundlesApi to get bundle transactions
      const bundlesApi = (await import('./bundlesApi.js')).default
      
      // Get all bundle transactions
      const allBundleTransactions = await firebaseDB.read('bundle_transactions')
      
      // Get all rooms to resolve room numbers
      const roomsApi = (await import('./roomsApi.js')).default
      const allRooms = await roomsApi.getAll()
      const roomMap = {}
      allRooms.forEach(room => {
        roomMap[room.id] = room.roomNumber || room.number || room.id
      })
      
      // Filter and transform bundle transactions for this item
      const bundleTransactions = []
      if (allBundleTransactions) {
        Object.entries(allBundleTransactions).forEach(([id, transaction]) => {
          if (transaction.consumedItems) {
            // Check if this item was consumed in this transaction
            const consumedItem = transaction.consumedItems.find(item => item.itemId === itemId)
            if (consumedItem) {
              const roomNumber = roomMap[transaction.roomId] || transaction.roomId
              bundleTransactions.push({
                id,
                type: 'bundle-consumption',
                itemId: itemId,
                itemName: consumedItem.name,
                quantity: -consumedItem.consumed, // Negative for consumption
                previousStock: consumedItem.previousStock,
                newStock: consumedItem.newStock,
                reason: `Bundle consumption - Room ${roomNumber}`,
                roomId: transaction.roomId,
                roomNumber: roomNumber,
                bundleName: transaction.bundleName,
                performedBy: transaction.inspectedBy,
                performedByName: 'Housekeeping Staff', // Will be resolved later
      ...
      // Get regular inventory transactions (if they exist)
      const regularTransactions = await firebaseDB.read(`inventory_transactions/${itemId}`) || []
      const formattedRegularTransactions = Array.isArray(regularTransactions) 
        ? regularTransactions 
        : Object.entries(regularTransactions).map(([id, txn]) => ({ id, ...txn }))

      // Combine and sort by date (newest first)
      const allTransactions = [...bundleTransactions, ...formattedRegularTransactions]
      allTransactions.sort((a, b) => new Date(b.createdAt) - new Date(a.createdAt))

      // Resolve user names for bundle transactions
      const usersApi = (await import('./usersApi.js')).default
      ...
      // Update performedByName with actual user names
      allTransactions.forEach(txn => {
        if (txn.performedBy && userMap[txn.performedBy]) {
          txn.performedByName = userMap[txn.performedBy].name || userMap[txn.performedBy].email
        }
      })
  ...
  async createAssetInstance(masterItemId, assetData) {
    ...
    try {
      ...
      const assetInstance = {
        ...
        name: masterItem.name, // Add name field
        type: 'asset-instance',
        ...
        currentStock: 1, // Each instance represents 1 unit
        restockThreshold: 0,
        maxStock: 1,
