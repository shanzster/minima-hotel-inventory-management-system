...
export async function fetchAudits(filters = {}) {
  try {
    ...
    const queryParams = new URLSearchParams(filters).toString()
    return await apiRequest(`/audits${queryParams ? `?${queryParams}` : ''}`)
  } catch (error) {
...
export async function fetchAdjustmentRequests(filters = {}) {
  try {
    ...
    const queryParams = new URLSearchParams(filters).toString()
    return await apiRequest(`/adjustment-requests${queryParams ? `?${queryParams}` : ''}`)
  } catch (error) {
...
export async function createAdjustmentRequest(requestData) {
  try {
    ...
    return await apiRequest('/adjustment-requests', {
      method: 'POST',
      body: requestData
    })
  } catch (error) {
