...
export function logout() {
  if (typeof window !== 'undefined') {
    // First, mark that user has logged out to prevent auto-login
    localStorage.setItem('hasLoggedOut', 'true')

    // Then clear all auth-related data from localStorage
    localStorage.removeItem('currentUser')

    // Clear any session-related data that might cause auto-login
    sessionStorage.clear()

    // Optional: Clear any other auth-related localStorage keys
